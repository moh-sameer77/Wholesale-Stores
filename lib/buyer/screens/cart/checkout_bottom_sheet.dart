...
class _CheckoutBottomSheetState extends State<CheckoutBottomSheet> {
        ...
        children: <Widget>[
          ...
          Consumer<Cart>(
            builder: (context, cart, child) {
              // Use cart object and rebuild widget tree based on changes

              return checkoutRow("Total Cost", trailingText: "\$ ${cart.totalPrice.toStringAsFixed(2)}");
  ...
  void createOrder(BuildContext context) {
    ...
    final ordersList = Provider.of<OrdersList>(context, listen: false);

    Order order = Order.fromCart(cart);
    submitOrder(order.itemsList);
    ordersList.addOrder(order);
    for( int i =0; i<ordersList.count; i++)
    print(ordersList.orders[i].items[0].name);
    ...
    //   context,
    //   MaterialPageRoute(
    //     builder: (context) => OrdersScreen(orders: [order]),
    //   ),
    // );
  }
}
